	      ...
	      <section class="content">
	        <div class="row">
	            ...
	            <?php
	       			...
	       			$stmt = $conn->prepare("SELECT COUNT(*) AS numrows FROM products WHERE id || name LIKE :keyword");
	       			...
	       			if($row['numrows'] < 1){
		       			...
		       			try{
		       			 	$inc = 3;	
						    $stmt = $conn->prepare("SELECT * FROM products WHERE id LIKE :keyword");
						    $stmt->execute(['keyword' => '%'.$_POST['keyword'].'%']);
					 
						    foreach ($stmt as $row) {
						    	$highlighted = preg_filter('/' . preg_quote($_POST['keyword'], '/') . '/i', '<b>$0</b>', $row['id']);
						    	$image = (!empty($row['photo'])) ? 'images/'.$row['photo'] : 'images/noimage.jpg';
						    	$inc = ($inc == 3) ? 1 : $inc + 1;
	       						if($inc == 1) echo "<div class='row'>";
	       						echo "
	       							<div class='col-sm-4'>
	       								<div class='box box-solid'>
		       								<div class='box-body prod-body'>
		       									<img src='".$image."' width='100%' height='230px' class='thumbnail'>
		       									<h5><a href='product.php?product=".$row['slug']."'>".$row['name']. ' (' . $highlighted . ')'."</a></h5>
		       								</div>
		       								<div class='box-footer'>
		       									<b>&#163; ".number_format($row['price'], 2)."</b>
		       								</div>
